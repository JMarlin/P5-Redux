...
S2_SIZE=$((`cat stage2_fat12.bin | wc -c`))
...
dd if=/dev/zero bs=1 count=$S2_PAD >>  pboot_fat12.img
...
for ((findex=0; findex<FATCOPIES; findex++)); do
    ...
    echo -n -e '\xF0\xFF\xFF\xFF\xFF\xFF' >>  pboot_fat12.img
    dd if=/dev/zero bs=1 count=$((SECTORSZ - 6)) >>  pboot_fat12.img
    dd if=/dev/zero bs=$SECTORSZ count=$((SECTPERFAT - 1)) >>  pboot_fat12.img
done

#Write the root directory entry starting with the volume label
#each entry is 32 bytes of info
echo -n 'PBOOT_R2   ' >>  pboot_fat12.img        #Text for volume label
echo -n -e '\x08' >>  pboot_fat12.img             #Attributes: 0x08 is volume label
dd if=/dev/zero bs=1 count=20 >>  pboot_fat12.img #The rest of the entry should be null
...
echo -n 'SUCCESS TXT' >>  pboot_fat12.img         #8.3 file name
dd if=/dev/zero bs=1 count=15 >>  pboot_fat12.img  #Don't need any of these 15 bytes
echo -n -e '\x02\x00' >>  pboot_fat12.img          #Start cluster, little-endian
echo -n -e `echo \\\x${TXLENS:2:2}\\\x${TXLENS:0:2}\\\x00\\\x00` >>  pboot_fat12.img  #Filesize, little-endian (one sect)

#Now fill out the rest of the directory entry
dd if=/dev/zero bs=32 count=$((ROOTENTRIES - 2)) >>  pboot_fat12.img

#Write the first cluster (cluster 2) with the message info
echo -n $MESSAGE >>  pboot_fat12.img

#Pad the rest of the sector
TXTPAD=$((SECTORSZ - TXTLEN))
dd if=/dev/zero bs=1 count=$TXTPAD >>  pboot_fat12.img
...
dd if=/dev/zero bs=$((SECTORSZ)) count=$((TOTALSECT - (S2_SECTORS + (SECTPERFAT * FATCOPIES) + 3))) >>  pboot_fat12.img
