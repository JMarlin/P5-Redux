...
void writePCIConfigReg(unsigned char bus, unsigned char device, unsigned char function, unsigned char reg, unsigned int value) {
	...
	while(1) {
		...
		switch(temp_msg.command) {
            ...
            case PCI_WRITEREG_30:
                writePCIConfigReg(
                    PCI_ADDR_BUS(pci_device[((unsigned int*)temp_msg.payload)[0]]),
                    PCI_ADDR_DEV(pci_device[((unsigned int*)temp_msg.payload)[0]]),
                    PCI_ADDR_FUNC(pci_device[((unsigned int*)temp_msg.payload)[0]]),
                    temp_msg.command - PCI_WRITEREG,
                    ((unsigned int*)temp_msg.payload)[1]
                );
                postMessage(temp_msg.source, temp_msg.command, 1);
                break;
